...
static int rt_init_phased(HWord values_needed)
{
...
{
	IRExpr* const_values_needed = IRExpr_Const(IRConst_HWord(values_needed));
	return unsafeIRDirty_1_N(phase, "init_phased", &rt_init_phased, mkIRExprVec_1(const_values_needed));
}

static void rt_set_phase(HWord phase)
{
	ML_(helper_my_state)()->phase = phase;
}

IRDirty* ML_(helper_set_phase)(HWord phase)
{
	IRExpr* const_phase = IRExpr_Const(IRConst_HWord(phase));
	return unsafeIRDirty_0_N("set_phase", &rt_set_phase, mkIRExprVec_1(const_phase));
}

#define accessor(Type, CType) \
static CType rt_get_##Type(HWord idx) \
{ \
	SRThreadState* state = ML_(helper_my_state)(); \
	tl_assert(0 <= idx && idx < state->values_used); \
	return state->values[idx].##Type; \
} \
\
static void rt_put_##Type(HWord idx, CType value) \
{ \
	SRThreadState* state = ML_(helper_my_state)(); \
	tl_assert(0 <= idx && idx < state->values_used); \
...
Accessor accessors[] = {
	{ Ity_U1, "get_U1", &rt_get_U1, "put_U1", &rt_put_U1 },
	{ Ity_U8, "get_U8", &rt_get_U8, "put_U8", &rt_put_U8 },
	{ Ity_U16, "get_U16", &rt_get_U16, "put_U16", &rt_put_U16 },
	{ Ity_U32, "get_U32", &rt_get_U32, "put_U32", &rt_put_U32 },
	{ Ity_U64, "get_U64", &rt_get_U64,"put_U64",  &rt_put_U64 },
...
IRDirty* ML_(helper_retrieve_temp)(IRTemp destination, IRTemp canonical, IRType type)
{
	...
	IRExpr* const_canonical = IRExpr_Const(IRConst_HWord(canonical));
	return unsafeIRDirty_1_N(destination, accessor->get_name, accessor->get, mkIRExprVec_1(const_canonical));
}

IRDirty* ML_(helper_store_temp)(IRTemp source, IRTemp canonical, IRType type)
{
	Accessor* accessor = get_accessor(type);
	IRExpr* const_canonical = IRExpr_Const(IRConst_HWord(canonical));
	IRExpr* temp_source = IRExpr_RdTmp(source);
	return unsafeIRDirty_0_N(accessor->put_name, accessor->put, mkIRExprVec_2(const_canonical, temp_source));
}
