...
IRExpr* ML_(instrument_expression)(SRState* state, IRExpr* expr)
{
	switch (expr->tag) {
		case Iex_Get:
			return expr;
		case Iex_GetI:
			expr->Iex.GetI.ix = ML_(instrument_expression)(state, expr->Iex.GetI.ix);
			return expr;
		case Iex_RdTmp:
			return instrument_temp(state, expr->Iex.RdTmp.tmp);
		case Iex_Qop:
			expr->Iex.Qop.details->arg1 = ML_(instrument_expression)(state, expr->Iex.Qop.details->arg1);
			expr->Iex.Qop.details->arg2 = ML_(instrument_expression)(state, expr->Iex.Qop.details->arg2);
			expr->Iex.Qop.details->arg3 = ML_(instrument_expression)(state, expr->Iex.Qop.details->arg3);
			expr->Iex.Qop.details->arg4 = ML_(instrument_expression)(state, expr->Iex.Qop.details->arg4);
			return expr;
		case Iex_Triop:
			expr->Iex.Triop.details->arg1 = ML_(instrument_expression)(state, expr->Iex.Triop.details->arg1);
			expr->Iex.Triop.details->arg2 = ML_(instrument_expression)(state, expr->Iex.Triop.details->arg2);
			expr->Iex.Triop.details->arg3 = ML_(instrument_expression)(state, expr->Iex.Triop.details->arg3);
			return expr;
		case Iex_Binop:
			expr->Iex.Binop.arg1 = ML_(instrument_expression)(state, expr->Iex.Binop.arg1);
			expr->Iex.Binop.arg2 = ML_(instrument_expression)(state, expr->Iex.Binop.arg2);
			return expr;
		case Iex_Unop:
			expr->Iex.Unop.arg = ML_(instrument_expression)(state, expr->Iex.Unop.arg);
			return expr;
		case Iex_Load:
		{
			IRExpr* orig_addr = ML_(instrument_expression)(state, expr->Iex.Load.addr);
			// TODO: This should be GWord, but for now we support only HWord==GWord
			IRExpr* addr = IRExpr_Mux0X(state->one_current_phase, mkIRExpr_HWord((HWord)&rt_dummy_location), orig_addr);
		...
		case Iex_Const:
			return expr;
		case Iex_Mux0X:
			expr->Iex.Mux0X.cond = ML_(instrument_expression)(state, expr->Iex.Mux0X.cond);
			expr->Iex.Mux0X.expr0 = ML_(instrument_expression)(state, expr->Iex.Mux0X.expr0);
			expr->Iex.Mux0X.exprX = ML_(instrument_expression)(state, expr->Iex.Mux0X.exprX);
			return expr;
		case Iex_CCall:
			{
				int i;
				for(i=0; expr->Iex.CCall.args[i] != NULL; i++)
					expr->Iex.CCall.args[i] = ML_(instrument_expression)(state, expr->Iex.CCall.args[i]);
				return expr;
			}
		default:
...
void ML_(instrument_statement)(SRState* state, IRStmt* stmt)
{
	switch (stmt->tag) {
		...
		case Ist_AbiHint:
			...
			break;
		case Ist_Put:
			stmt->Ist.Put.data = ML_(instrument_expression)(state, stmt->Ist.Put.data);
			...
			break;
		case Ist_PutI:
			stmt->Ist.PutI.details->ix = ML_(instrument_expression)(state, stmt->Ist.PutI.details->ix);
			stmt->Ist.PutI.details->data = ML_(instrument_expression)(state, stmt->Ist.PutI.details->data);
		...
		case Ist_WrTmp:
			...
			stmt->Ist.WrTmp.data = ML_(instrument_expression)(state, stmt->Ist.WrTmp.data);
			addStmtToIRSB(state->sb, stmt);
			break;
		case Ist_Store:
			stmt->Ist.Store.addr = ML_(instrument_expression)(state, stmt->Ist.Store.addr);
			stmt->Ist.Store.addr = ML_(flatten_expression)(state->sb, IRExpr_Mux0X(state->one_current_phase, mkIRExpr_HWord((HWord)&rt_dummy_location), stmt->Ist.Store.addr));
			stmt->Ist.Store.data = ML_(instrument_expression)(state, stmt->Ist.Store.data);
			addStmtToIRSB(state->sb, stmt);
			break;
		case Ist_CAS:
			stmt->Ist.CAS.details->addr = ML_(instrument_expression)(state, stmt->Ist.CAS.details->addr);
			stmt->Ist.CAS.details->addr = ML_(flatten_expression)(state->sb, IRExpr_Mux0X(state->one_current_phase, mkIRExpr_HWord((HWord)&rt_dummy_location), stmt->Ist.CAS.details->addr));
			stmt->Ist.CAS.details->expdLo = ML_(instrument_expression)(state, stmt->Ist.CAS.details->expdLo);
			if (stmt->Ist.CAS.details->expdHi != NULL)
				stmt->Ist.CAS.details->expdHi = ML_(instrument_expression)(state, stmt->Ist.CAS.details->expdHi);
			stmt->Ist.CAS.details->dataLo = ML_(instrument_expression)(state, stmt->Ist.CAS.details->dataLo);
			if (stmt->Ist.CAS.details->dataHi != NULL)
				stmt->Ist.CAS.details->dataHi = ML_(instrument_expression)(state, stmt->Ist.CAS.details->dataHi);
		...
		case Ist_Dirty:
			if (stmt->Ist.Dirty.details->tmp == IRTemp_INVALID) {
				IRExpr* orig_guard = IRExpr_Unop(Iop_1Uto8, ML_(instrument_expression)(state, stmt->Ist.Dirty.details->guard));
...
//				stmt->Ist.Dirty.details->guard = ML_(flatten_expression)(state->sb, IRExpr_Mux0X(state->one_current_phase, IRExpr_Const(IRConst_U1(False)), stmt->Ist.Dirty.details->guard));
				...
				for(i=0;stmt->Ist.Dirty.details->args[i]!=NULL;i++)
					stmt->Ist.Dirty.details->args[i] = ML_(instrument_expression)(state, stmt->Ist.Dirty.details->args[i]);
			} else {
		...
		case Ist_Exit:
		{
			...
			IRExpr* orig_guard = IRExpr_Unop(Iop_1Uto8, ML_(instrument_expression)(state, stmt->Ist.Exit.guard));
			IRExpr* new_guard = IRExpr_Binop(Iop_And8, orig_guard, state->one_current_phase);
			stmt->Ist.Exit.guard = ML_(flatten_expression)(state->sb, IRExpr_Unop(Iop_32to1, IRExpr_Unop(Iop_8Uto32, new_guard)));
			ML_(instrument_phased_exit)(state, stmt->Ist.Exit.guard);
			addStmtToIRSB(state->sb, stmt);
			break;
		}
		default:
...
{
	SRState state;
	state.self_address = IRConst_U64(self_address);

	state.sb = sb;
	state.temps_count = sb->tyenv->types_used;
